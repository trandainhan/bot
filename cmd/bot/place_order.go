...
func placeOrder(botID string, quantity, exchancePrice, fiahubPrice float64, side string) {
	...
	text := fmt.Sprintf("%s %s Take profit From Exchange %s Quant: %v Price: %v ID: %d", coin, botID, side, quantity, exchancePrice, ID)
...
func notifyFailedOrder(botID string, sellQuantity, totalUSDT float64, orderType string, err error) {
	text := fmt.Sprintf("Error %s: Can not make order on Exchange %s %s", os.Getenv("TELEGRAM_HANDLER"), coin, botID)
	text = fmt.Sprintf("%s =========================: %sLIMIT: Quantity: %v TotalUSDT %v Error: %s", text, orderType, sellQuantity, totalUSDT, err)
	log.Println(text)
